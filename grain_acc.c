...
void initialize_occupancy(int occupancy[n][n],float rand_matrix[n][n]) {
  int i, j;
  ...
  for (i=0; i<n; i++) {
   for (j=0; j<n; j++) {
    rand_matrix[i][j] = random() / (float) RAND_MAX;
   }
  }
}
...
void hop(int occupancy[n][n],float rand_matrix[n][n]) {
  int i,j;
  ...
  #pragma acc parallel loop collapse(2) present(occupancy[n][n]) present(rand_matrix[n][n])
  for (i=0; i<n; i++) {
   for (j=0; j<n; j++) {
      // cell is occupied
      if (occupancy[i][j] == 1) {
        // desportion
        float sample = rand_matrix[i][j];
        ...
        // diffusion
          ...
          int random_index = (int) (sample * 10) % 8;
...
void run_sim (int occupancy[n][n],float rand_matrix[n][n]) // Run full simulation for given number of timesteps
{
  double next_adsorption_time = 0.0; // Next timestep at which a hydrogen particle would be added

  // Set up loop through timesteps
  #pragma acc data copyin(occupancy[0:n][0:n]) copyin(rand_matrix[0:n][0:n])
...
int main (int argc, char *argv[])
{
  ...
  initialize_occupancy(occupancy,rand_matrix);
  run_sim(occupancy,rand_matrix);
